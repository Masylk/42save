...
double	check_caps(double t_one, t_ray ray, t_cyl *cyl)
{
	...
	if (t < 0 || dot_product(denom, denom)
	...
	if (t_one < 0 || dot_product(denom, denom)
			< ((cyl->width * 0.5) * (cyl->width * 0.5)))
		return (t);
	if (t > t_one)
		t = t_one;
...
double	check_disc(double d, t_ray ray, t_cyl *cyl)
{
	double	sq;
	double	t_one;
	double	t_two;
...
//faire une structure pour les variables des équations
// a,b,c,d
// t,q,p, t_one, t_two
// v, r
...
int	check_cylinder(t_cyl *cyl, t_ray ray)
{
	...
	v = sub(ray.direction,
			mul(dot_product(ray.direction, cyl->v), cyl->v));
	d = sub(ray.origin, cyl->coor);
	a = dot_product(v, v);
	b = dot_product(v, sub(d, mul(cyl->v, dot_product(d, cyl->v)))) * 2.0;
	v = sub(d, mul(cyl->v, dot_product(d, cyl->v)));
	c = dot_product(v, v) - ((cyl->width * 0.5) * (cyl->width * 0.5));
	d = (b * b) + (4 * a * c);
	if (d < 0)
		return (0);
	a = check_disc(d, ray, cyl);
	if (a < 0)
		return (0);
	return (1);
}
